            ...
            if current != q_goal:
                for q_n in knn(current,Q_v,N_knn):
                    if (not checkCollision(current, q_n, obstacles)):
                        pixel_distance[Q_v.index(q_n)] = pixel_distance[Q_v.index(current)] + dist(q_n, current)
...
#     connectNeighbors(q_n_start,start,'orange')

#     q_n_goal = knn(goal, Q_valid, N_knn)
#     connectNeighbors(q_n_goal,goal,'cyan')

    #dist, parent = a_star(start, goal, Q_valid, obstacles)
